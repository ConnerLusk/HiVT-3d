...
from utils import TemporalData, rotation_matrix_from_angles
...
class HiVT(pl.LightningModule):
    ...
    def forward(self, data: TemporalData):
        if self.rotate:
            rotate_mat = torch.empty(data.num_nodes, 3, 3, device=self.device)
            for i, angle in enumerate(data["rotate_angles"]):
                rotate_mat[i] = rotation_matrix_from_angles(
        ...
        y_hat, pi = self.decoder(local_embed=local_embed, global_embed=global_embed, numNodes=data.edge_index.shape[0])
        return y_hat, pi
    ...
    def training_step(self, data, batch_idx):
        ...
        l2_norm = (torch.norm(y_hat[:, :, :, :3] - data.y, p=2, dim=-1) * reg_mask).sum(
            dim=-1
        )  # [F, N]
    ...
    def validation_step(self, data, batch_idx):
        ...
        y_hat_agent = y_hat[:, data["agent_index"], :, :3]
