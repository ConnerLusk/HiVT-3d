...
class AAEncoder(MessagePassing):
                ...
                center_embed = self.center_embed(
                    torch.matmul(
                        x.view(
                            ...
                            x.shape[0] // self.historical_steps,
                            -1,
                        ).unsqueeze(-2),
    ...
    def message(
        ...
        key = self.lin_k(nbr_embed).view(
            -1, self.num_heads, self.embed_dim // self.num_heads
        )
        value = self.lin_v(nbr_embed).view(
            -1, self.num_heads, self.embed_dim // self.num_heads
        )
...
class TemporalEncoderLayer(nn.Module):
    def __init__(
        ...
        self.linear1 = nn.Linear(embed_dim, embed_dim * 6)
